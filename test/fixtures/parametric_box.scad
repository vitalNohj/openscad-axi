...
module box_body() {
  difference() {
    ...
    translate([wall_thickness, wall_thickness, wall_thickness])
      rounded_box(width - 2*wall_thickness, depth - 2*wall_thickness,
                  height, max(0, corner_radius - wall_thickness));
  }
}
...
module lid() {
  ...
  difference() {
    union() {
      ...
      translate([wall_thickness + lid_tolerance, wall_thickness + lid_tolerance,
                 -lip_height + wall_thickness])
        rounded_box(inner_w, inner_d, lip_height,
                    max(0, corner_radius - wall_thickness));
    }
    if (add_grip)
      for (x = [width * 0.3, width * 0.7], yoff = [-1, depth - 4])
        translate([x, yoff, wall_thickness/2])
          rotate([-90, 0, 0]) cylinder(h = 5, r = 3, $fn = 16);
  }
}
